...
const JobList: Category[] = [
  { label: "Deployment", total: "4,200+ jobs", icon: <Cloud size={28} /> },
  { label: "Design & Creative", total: "2,100+ jobs", icon: <Palette size={28} /> },
  { label: "Plumbering", total: "1,200+ jobs", icon: <Megaphone size={28} /> },
  { label: "Marketing", total: "4,200+ jobs", icon: <BarChart2 size={28} /> },
  { label: "Data & Analytics", total: "2,100+ jobs", icon: <Shield size={28} /> },
  { label: "CyberSecurity", total: "1,200+ jobs", icon: <Music2 size={28} /> },
  { label: "Electrician", total: "1,200+ jobs", icon: <Smartphone size={28} /> },
...
const HowItWorksSteps: Step[] = [
  ...
  },
  {
    ...
    icon: (
      ...
      <svg className="bg-secondary/8 w-22 h-22 rounded-xl flex items-center justify-center text-primary shrink-0 p-6" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M61.2125 61.2123L44.0971 44.0968M44.0971 44.0968C48.7294 39.4645 51.3318 33.1817 51.3318 26.6306C51.3318 20.0795 48.7294 13.7967 44.0971 9.16442C39.4647 4.5321 33.182 1.92969 26.6309 1.92969C20.0798 1.92969 13.797 4.5321 9.16467 9.16442C4.53234 13.7967 1.92993 20.0795 1.92993 26.6306C1.92993 33.1817 4.53234 39.4645 9.16467 44.0968C13.797 48.7291 20.0798 51.3315 26.6309 51.3315C33.182 51.3315 39.4647 48.7291 44.0971 44.0968Z" stroke="#7C3AED" stroke-width="3.86" stroke-linecap="round" stroke-linejoin="round" />
      </svg>
        ...
        <Hero />
            ...
            <section className="py-14 sm:py-20 px-5 sm:px-8 md:px-16 text-center">
              ...
              <p className="text-muted text-sm sm:text-[18px] max-w-md mx-auto mb-12 sm:mb-16">
                ...
                {/* Division */}
                <div className="hidden md:flex items-center justify-center gap-9 p-5 bg-card-foreground mx-98 my-9 rounded-xl">
                  {/* tenenat */}
                  <div className="bg-background py-2 px-5 rounded-lg border border-[#f3f3fe] text-primary">
                    <p>For Talents</p>
                  </div>
                  {/* Employers */}
            ...
            {/* ── The Future Network ── */}
            <section
              className="relative mx-4 sm:mx-8 md:mx-16 rounded-3xl overflow-hidden min-h-[320px] sm:min-h-[420px] flex flex-col items-center justify-center text-center px-6 py-16 sm:py-24"
            >
              {/* Global */}
              <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-[500px] h-[200px] rounded-full" />
                <h2 className=" text-primary text-shadow font-bold text-2xl sm:text-3xl md:text-4xl leading-tight mb-4">
                  The Future Network For<br className="hidden sm:block" /> Skilled Talent
                </h2>
              <div className="relative w-full max-w-full flex justify-center overflow-hidden px-0">
                  <div className="absolute inset-x-0 top-0 flex flex-col justify-center gap-4 pointer-events-none">
                    <img
                      src="/purpleFrame.png"
                      className="w-[295px] max-w-[100%] ml-[50rem] mt-15 object-contain"
                      alt="Frame 1"
                    />
                    <img
                      src="/blueFrame.png"
                      className="w-[295px] max-w-[100%] object-contain"
                      alt="Frame 2"
                    />
                  </div>
            ...
            <div style={{background:"url('/Mask-group.png')"}} className="text-background w-full h-90 flex flex-col justify-center gap-7 items-center">
              <h2 className="font-semibold text-3xl md:text-4xl">Ready to get Started?</h2>
              <p className="hidden md:flex text-xl w-120 text-center">Join 89,000+ talents and 1,800+ companies already on the platform.</p>
              <p className=" text-[14px] w-120 text-center md:hidden">Join 89,000+ talents already on the platform</p>
              {/* Buttons */}
              <div className="hidden md:flex flex-col gap-9 items-center md:flex-row ">
                <Link className="bg-background px-6 py-3 text-primary font-semibold rounded-lg" href={PATHS.GetStarted}>Sign up as a Talent</Link>
                <Link className=" px-15 rounded-lg md:rounded-lg backdrop-blur-2xl bg-white/10 border border-white/10 px-6 py-3" href={PATHS.GetStarted}>Post a Job</Link>
              </div>
              <Link className="bg-background px-6 py-3 text-primary font-semibold rounded-lg md:hidden" href={PATHS.GetStarted}>Sign up Free</Link>
            </div>
            <Footer />
       </div>
  )
